...
export function setSessionCookie(res: Response, payload: { userId: string; email: string; role: 'user' | 'admin' }) {
...
export async function requireAuth(req: Request, _res: Response, next: NextFunction) {
  ...
  try {
    const decoded = jwt.verify(token, env.SESSION_SECRET) as { email: string; role: 'user' | 'admin'; sub?: string };
