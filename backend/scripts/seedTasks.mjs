...
const taskTemplates = {
  sales: [
    ...
    { name: 'Prepare sales presentation for ABC Corp', desc: 'Create deck showcasing our enterprise solutions' },
    { name: 'Update CRM with client feedback', desc: 'Input notes from recent client calls into Salesforce' },
    { name: 'Quarterly sales report', desc: 'Compile revenue data and forecasts for Q1' },
    { name: 'Client renewal negotiations', desc: 'Schedule meetings with clients up for contract renewal' },
  ...
  it: [
    ...
    { name: 'Database backup verification', desc: 'Confirm all automated backups are running correctly' },
    ...
    { name: 'Network infrastructure upgrade', desc: 'Plan and execute switch replacement in Building B' },
    { name: 'User access audit', desc: 'Review and clean up inactive user accounts' },
  ],
  finance: [
    { name: 'Monthly expense reconciliation', desc: 'Match credit card statements with expense reports' },
    { name: 'Budget forecast for Q2', desc: 'Prepare financial projections for next quarter' },
    { name: 'Vendor payment processing', desc: 'Process outstanding invoices and approve payments' },
    ...
    { name: 'Financial report for board meeting', desc: 'Create comprehensive financial summary for executives' },
  ],
  marketing: [
    { name: 'Social media content calendar', desc: 'Plan and schedule posts for next month' },
    { name: 'Launch campaign for new product', desc: 'Coordinate email, social, and paid ads for Product X' },
    ...
    { name: 'Event planning for trade show', desc: 'Organize booth, materials, and staff for industry conference' },
  ...
  hr: [
    ...
    { name: 'Performance review cycle kickoff', desc: 'Launch annual performance evaluation process' },
    { name: 'Update employee handbook', desc: 'Revise policies to reflect new remote work guidelines' },
  ...
  'customer-service': [
    ...
    { name: 'Knowledge base article updates', desc: 'Refresh FAQ and troubleshooting documentation' },
...
async function generateTasks(userList) {
  ...
  for (let i = 0; i < nearTermCount; i++) {
    ...
    const task = {
      ...
      dueDate,
      status,
      completedAt: status === 'Completed' ? new Date(dueDate.getTime() - Math.random() * 7 * 24 * 60 * 60 * 1000) : undefined,
    };
...
async function seedTasks() {
  try {
    ...
    console.log(`  - Priorities: High (${tasks.filter(t => t.priority === 'high').length}), Medium (${tasks.filter(t => t.priority === 'medium').length}), Low (${tasks.filter(t => t.priority === 'low').length})`);
